...
@section('content')
    <div class="container mx-auto p-4 max-w-2xl">
        <div class="flex items-center justify-between mb-4">
            <h1 class="text-2xl font-semibold">Editar Etiqueta</h1>
        ...
        <form method="POST" action="{{ route('etiquetas.update', $etiqueta) }}" class="space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label class="block font-medium mb-1" for="nombre">Nombre *</label>
                <input id="nombre" name="nombre" type="text" value="{{ old('nombre', $etiqueta->nombre) }}"
                    class="w-full rounded border p-2" maxlength="80" required>
