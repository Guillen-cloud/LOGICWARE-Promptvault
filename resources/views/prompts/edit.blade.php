...
@section('content')
    <div class="container mx-auto p-4 max-w-3xl">
        <div class="flex items-center justify-between mb-4">
            <h1 class="text-2xl font-semibold">Editar Prompt</h1>
            <a href="{{ route('prompts.show', $prompt) }}" class="rounded border px-3 py-2 hover:bg-gray-50">Volver</a>
        </div>
        ...
        <form action="{{ route('prompts.update', $prompt) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label class="block font-medium mb-1" for="titulo">Título *</label>
                <input id="titulo" name="titulo" type="text" value="{{ old('titulo', $prompt->titulo) }}"
                    class="w-full rounded border p-2" maxlength="180" required>
            ...
            <div>
                ...
                <input id="ia_destino" name="ia_destino" type="text" value="{{ old('ia_destino', $prompt->ia_destino) }}"
            ...
            <div>
                ...
                <textarea id="descripcion" name="descripcion" class="w-full rounded border p-2"
                    rows="3">{{ old('descripcion', $prompt->descripcion) }}</textarea>
            ...
            <div>
                ...
                <textarea id="contenido" name="contenido" class="w-full rounded border p-2" rows="10"
                    required>{{ old('contenido', $prompt->contenido) }}</textarea>
                @error('contenido') <p class="text-red-700 text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            {{-- ✅ NUEVO: Motivo del cambio (solo en editar) --}}
            <div>
                <label class="block font-medium mb-1" for="motivo_cambio">Motivo del cambio *</label>
                <input id="motivo_cambio" name="motivo_cambio" type="text" value="{{ old('motivo_cambio') }}"
                    class="w-full rounded border p-2" maxlength="255" required
                    placeholder="Ej: Mejoré el prompt, corregí un error, agregué contexto...">
                @error('motivo_cambio') <p class="text-red-700 text-sm mt-1">{{ $message }}</p> @enderror
            </div>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('prompts.show', $prompt) }}" class="rounded border px-4 py-2 hover:bg-gray-50">
                    Cancelar
                </a>
            </div>
        </form>
